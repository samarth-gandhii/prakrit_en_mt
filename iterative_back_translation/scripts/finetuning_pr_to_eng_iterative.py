"""Finetuning IndicTrans2 for Prakrit→English with continued fine-tuning support.

This is a modified copy of the original finetuning script that supports loading
model weights from a SEPARATE path (e.g., a previously fine-tuned model like
models/prakrit_to_eng_v2-final) while loading the tokenizer from the base model.

Key difference from the original:
  --pretrained_weights: If provided, model is loaded from here instead of --base_model.
                        Tokenizer is ALWAYS loaded from --base_model.
"""
...
def create_source_target_pairs(lines):
    ...
    for line in lines:
        ...
        # For Prakrit -> English: Source MUST be Prakrit (Devanagari) and Target MUST be English
...
def main():
    ...
    parser = ArgumentParser(description='Finetuning IndicTrans2 for Prakrit→English (with continued fine-tuning support).')
    ...
    parser.add_argument(
        ...
        help='Path to pre-trained model weights to continue fine-tuning from '
             '(e.g., models/prakrit_to_eng_v2-final). '
